...
Выбор модели для распознавания речи. Модели отличаются размером, скоростью и точностью. Для русского актуальны `gigaam-v3-ctc` [865/220], `vosk-model-ru` [260/75], `vosk-model-small-ru` [90/25]. В скобках размеры для fp32 и int8. Для английского может быть интересна `parakeet-tdt-0.6b-v2` [2.4gb/650]. Мультиязычная `nemo-canary-1b-v2` [3.5gb/980]

### Опция: `int8_quantization`

Выбор int8 квантизации модели. Снижаем размер используемой памяти, немного жертвуя точностью. На скорость выполнения обычно не влияет. Доступно не для всех моделей.
